...
// 부가적인 작업(side effect)을 하라고 있는 훅인 useEffect쓰고
    ...
    function onScroll(){
        const crrScrollY = window.scrollY;  //현 스크롤 위치
        // console.log(window.scrollY);

        
        if(crrScrollY > lastScrollY.current){   //직전
            setScrollShow(false);
       ...
       lastScrollY.current = crrScrollY;
    }
    window.addEventListener('scroll',onScroll);//스크롤할때마다 실행

    return ()=>{
        window.removeEventListener('scroll', onScroll); //해제
    }
    },[lastScrollY]);//lastScrollY 바뀌면 ef재실행
